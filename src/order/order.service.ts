...
export class OrdersService {
    ...
    async create(createOrderDto: CreateOrderDto): Promise<OrderResponseDto> {
        ...
        const totalAmountPending = createOrderDto.totalAmount - (createOrderDto.totalAmountPaid || 0);
        if (totalAmountPending < 0) {
            throw new BadRequestException('El monto pagado no puede ser mayor al monto total');
        }
        ...
        if (createOrderDto.details?.length > 0) {
            order.details = createOrderDto.details.map(detail => {
                ...
                orderDetail.productVariantId = detail.productVariantId;
    ...
    async update(id: string, updateOrderDto: UpdateOrderDto): Promise<OrderResponseDto> {
        ...
        const totalAmountPending = updateOrderDto.totalAmount - updateOrderDto.totalAmountPaid;
        if (totalAmountPending < 0) {
            throw new BadRequestException('El monto pagado no puede ser mayor al monto total');
        }
        ...
        if (updateOrderDto.details?.length > 0) {
            order.details = updateOrderDto.details.map(detail => {
                const existingDetail = order.details.find(d => d.productVariantId === detail.productVariantId);
                if (existingDetail) {
